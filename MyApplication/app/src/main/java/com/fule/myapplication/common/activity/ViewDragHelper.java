 ...
 * viewdraghelper是写入自定义viewgroup实用类。它提供了一个
 * 允许用户拖动的有用操作和状态跟踪的数量
 * 复位的观点在它们的父ViewGroup。
 */
public class ViewDragHelper {
     ...
     * 启用父视图的选定边缘的边缘跟踪。这个
     *回调的
     * {@链接viewdraghelper。回调# onedgetouched（int，int）}
     *
     * {@链接viewdraghelper。回调# onedgedragstarted（int，int）}
     *方法只会被调用边缘跟踪已被
     ...
     * event stream.
     *检查是否提供给父视图的此事件
     * onintercepttouchevent应引起家长拦截触摸
     *事件流。
        ...
        switch (action) {
            ...
            case MotionEventCompat.ACTION_POINTER_UP: {
                ...
                if (mDragState == STATE_DRAGGING && pointerId == mActivePointerId) {
                    // Try to find_select another pointer that's still holding on to
                    // the captured view.
                    ...
                    if (newActivePointer == INVALID_POINTER) {
                        // We didn't find_select another pointer still touching the
                        // view, release it.
